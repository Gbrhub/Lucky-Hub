...
v23.Image = "rbxassetid://95800489232564";
...
Title = "Tab Updates Info"

}),

Banned = v15:AddTab({

Title = "Tab Bypass Safe"
...
Title = "Webhook JobId Server"

}),

Stats = v15:AddTab({

Title = "Tab Auto Stats"
...
Title = "Tab Teleport Sea"

}),

Visual = v15:AddTab({

Title = "Fake Fun Level"
...
-- blade hits 
function getAllBladeHits(range)
    local hits = {}
    ...
    for _, enemy in pairs(game.Workspace.Enemies:GetChildren()) do
        if enemy:FindFirstChild("HumanoidRootPart") and enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 then
            if (player.Character.HumanoidRootPart.Position - enemy.HumanoidRootPart.Position).Magnitude <= range then
                table.insert(hits, enemy.HumanoidRootPart)
            end
        end
    end
    ...
    for _, otherPlayer in pairs(game.Players:GetPlayers()) do
        if otherPlayer ~= player and otherPlayer.Character and otherPlayer.Character:FindFirstChild("HumanoidRootPart") and otherPlayer.Character:FindFirstChild("Humanoid") then
            if (player.Character.HumanoidRootPart.Position - otherPlayer.Character.HumanoidRootPart.Position).Magnitude <= range then
                table.insert(hits, otherPlayer.Character.HumanoidRootPart)
            end
        end
    end
...
function AttackFunction()
    ...
    if ac and ac.equipped then
        for indexincrement = 1, 1 do
            local bladehit = getAllBladeHits(60) -- Now includes players
            if #bladehit > 0 then
...
function _x1()
    ...
    local function _x3(_v6)
        local _v7 = game:GetService("Workspace").Enemies:GetChildren()
        local _v8 = {}
        ...
        for _, _v10 in ipairs(game:GetService("Players"):GetPlayers()) do
            if _v10 ~= _v1 and _x2(_v10.Character) then
                local _v11 = _v10.Character:FindFirstChild("HumanoidRootPart")
                if _v11 and (_v11.Position - _v9).Magnitude <= _v6 then
                    table.insert(_v8, _v10.Character)
                end
            end
        end
        return _v8
    ...
    if _v3:FindFirstChild("LeftClickRemote") then
        ...
        for _, _v14 in ipairs(_v13) do
            local _v15 = (_v14.HumanoidRootPart.Position - _v2:GetPivot().Position).Unit
            pcall(function()
        ...
        pcall(function()
            ...
            if _v23 and _v24 then
                ...
                warn("Error: RegisterAttack or RegisterHit RemoteEvent not found!")
            end
        end)
    end
end
...
Title = "Discord Lucky Hub",

Description = "Join For Updates And Fix And Suggestions (Click To Copy)",
...
Description = "Fast Farm With Dodge Skill Mobs",
                    ...
                    for _, enemy in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if enemy:FindFirstChild("Humanoid") and enemy:FindFirstChild("HumanoidRootPart") and enemy.Humanoid.Health > 0 then
                            if enemy.Name == Ms then
                                -- Keep tracking enemy position while it's alive
                                while _G.AutoLevel and enemy.Parent and enemy.Humanoid.Health > 0 and questGui.Visible do
                                    task.wait(0.1) -- Reduce lag and ensure smooth tracking

                                    -- Attack & auto farm logic
                                    AttackNoCoolDown()
                                    bringmob = true
                                    AutoHaki()
                                    ...
                                    -- Continuously move with the enemy
                                    local targetPos = enemy.HumanoidRootPart.Position + Vector3.new(0, 50, 0)
...
Description = "Sea Castle Mission Raid",
...
Title = "Auto Haki Fortress",

Description = "Aura Skin Color",
...
Title = "Auto Random Roll Bones",

Description = "Auto Spin Bone",
...
Title = "Auto Farm Cake Commander",
...
local v60 = v16.Main:AddSection("Boss Mobs");
...
local v63 = v16.Main:AddSection("Materials");
...
Description = "See Kitsune Island Distance",
...
spawn(function()
    ...
    -- Wait for Kitsune Island to exist
    repeat
            ...
            if v845 then
                for _, v1336 in pairs(v845:GetDescendants()) do
                    if v1336:IsA("BasePart") and v1336.Name:find("NeonShrinePart") then
                        -- Ensure Tween function exists and properly tweens once
                        if typeof(Tween) == "function" then
                            Tween(v1336.CFrame)
                        else
                            print("Error: Tween function not found")
                        end
                        _G.TweenToKitsune = false -- Stop after reaching
                        break
                    end
                end
            end
...
local v503 = v16.Sea:AddSection("Island Events");
...
Description = "Use Boat To Activate",
...
Description = "Use Boat And 5 Players To Find Leviathan Dimension",
...
Content = "Frozen Dimension Island Found",
...
local vLeviathan = v16.Sea:AddToggle("AutoKillLeviathanToggle", {
...
AutoKillLeviathanToggle:OnChanged(function(value)
    _G.Auto_Kill_Leviathan = value;
end);

AutoKillLeviathanToggle:SetValue(false);

spawn(function()
    while wait() do
        if _G.Auto_Kill_Leviathan then
            pcall(function()
                if game:GetService("Workspace").SeaBeasts:FindFirstChild("Leviathan") then
                    for _, leviathan in pairs(game:GetService("Workspace").SeaBeasts:GetChildren()) do
                        if leviathan.Name == "Leviathan" and leviathan:FindFirstChild("Humanoid") and leviathan:FindFirstChild("HumanoidRootPart") and leviathan.Humanoid.Health > 0 then
                            repeat
                                wait(_G.Fast_Delay);
                                AttackNoCoolDown();
                                AutoHaki();
                            ...
                            until not _G.Auto_Kill_Leviathan or not leviathan.Parent or leviathan.Humanoid.Health <= 0
                        end
                    end
                elseif game:GetService("ReplicatedStorage"):FindFirstChild("Leviathan") then
                    Tween(game:GetService("ReplicatedStorage"):FindFirstChild("Leviathan").HumanoidRootPart.CFrame * CFrame.new(2, 20, 2));
                end
            end);
        end
    end
...
Title = "Auto Teleport To Tiki Outpost",

Description = "Drive Boat To Teleport",
...
Title = "Auto Teleport To Hydra Island",
...
Description = "Buy Selected Boats",
...
Description = "Fly To Boat You Bought",
...
local vSeaKing = v16.Sea:AddToggle("AutoKillSeaKingToggle", {
...
AutoKillSeaKingToggle:OnChanged(function(value)
    _G.Auto_Kill_Sea_King = value;
end);

AutoKillSeaKingToggle:SetValue(false);

spawn(function()
    while wait() do
        if _G.Auto_Kill_Sea_King then
            pcall(function()
                if workspace.SeaBeasts:FindFirstChild("SeaBeast1") then
                    Tween(workspace.SeaBeasts:FindFirstChild("SeaBeast1").HumanoidRootPart.CFrame * CFrame.new(0, 460, 0));
...
v532:SetDesc("Mirage Island: " .. MirageStatus .. " | FullMoon: " .. FullMoonStatus);
...
Description = "Mirage Highest Point",
...
Title = "Auto Teleport Advance Fruit Dealer",
...
Description = "Not Recommended Sometimes Bug",
...
Title = "Enable Remove Notifications",
...
Description = "To Prevent Overheating For Only Afk Lol",
...
local v99 = v16.Home:AddParagraph({
...
local v100 = v16.Home:AddParagraph({
...
local v102 = v16.Home:AddParagraph({
...
local v104 = v16.Home:AddParagraph({
...
-- Create Toggle
local ToggleSpectate = v16.Player:AddToggle("ToggleSpectate", {
    Title = "Spectate Player",
    Description = "Enable or disable spectating",
    Default = false
})
...
Description = "Target Selected Player",
...
local v56 = v16.Hunt:AddSection("Setting Player");

local v115 = v16.Hunt:AddToggle("ToggleNoClip", {
...
local v116 = v16.Hunt:AddToggle("ToggleWalkonWater", {
...
local v118 = v16.Teleport:AddSection("Sea Worlds");

local v119 = v16.Teleport:AddToggle("ToggleAutoSea2", {

Title = "Auto Unlock Sea 2",
...
Description = "Tp To Selected Island",
...
Description = "Make Sure You Have Or Bought Perm Fruits",
...
Title = "Auto Switch Permanent Fruit",
...
Description = "Instant Collect Fruits Auto If There is Spawn Or Dropped Photo",
...
Title = "Teleport Collect Fruit",

Description = "Better No Report Ban",
...
local v56 = v16.ESP:AddSection("Esp Visuals");

local v130 = v16.ESP:AddToggle("ToggleEspPlayer", {
...
local v131 = v16.ESP:AddToggle("ToggleEspFruit", {
...
local v132 = v16.ESP:AddToggle("ToggleEspIsland", {
...
local v133 = v16.ESP:AddToggle("ToggleEspFlower", {
...
local v134 = v16.ESP:AddToggle("ToggleEspRealFruit", {

Title = "Esp Fruit Berries",
...
local v135 = v16.ESP:AddToggle("ToggleIslandMirageEsp", {
...
local RaidToggle = v16.Raid:AddToggle("ToggleMobAura", {

Title = "Auto Raid Mobs",

Description = "Kill Aura A Little Lol",
...
Description = "Unstore Under 1m Fruit In Storage",
...
Title = "Teleport To Raid Entrance",
...
local v56 = v16.Raid:AddSection("Law Raids");
...
Title = "Teleport To TempleClock",

Description = "Only Works When You Are At Temple Of Time",

Callback = function()

game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(29536.826171875, 15068.751953125, -86.30783081054688));

Tween2(CFrame.new(29536.826171875, 15068.751953125, -86.30783081054688));
...
local vCyborg = v16.Race:AddToggle("ToggleCyborgFly", {

Title = "Stop Flying Cyborg",

Description = "Use This To Stop Flying Auto Trial Only Cyborg Race (Cyborg Trial Will Tween Fly Outside So Stop it Before It Reach Long, Stop It When You Already Fly At Outside But Not Too Far In Trial Of Machine)",
...
Description = "Kill Players After Trial",
...
local v56 = v16.Shop:AddSection("Fighting Styles");
...
local v56 = v16.Shop:AddSection("Misc");
...
local v56 = v16.Misc:AddSection("Claim Codes");
...
local vInfo = v16.Banned:AddParagraph({

Title = "For Safety Purposes",

Content = "After You Turn On Anti Reset Account Please Make Sure You Won't Last 50 Minutes On The Game For Safety Purposes No Reset Account Or Ban"

});

local v153 = v16.Banned:AddToggle("ToggleAntiBand", {

Title = "Anti Reset Account",

Description = "Safe For Bounty Hunt",
...
Title = "Fly To Frozen Dimension Island",
...
Title = "Spy Talk Leviathan Status ",
...
Title = "Accept Red Fire Quest",

Description = "Turn it on once to receive it, then turn it off and on again to receive another.",
...
Title = "Chop Tree On Hydra Island",
...
Title = "Teleport / Fly To Dragon Dojo",
...
Title = "Complete Draco Trial Coming Soon",
...
Title = "Auto Event Prehistoric Volcano",
...
Title = "Auto Collect Bones Volcano",
...
local v178 = v16.Hunt:AddToggle("SuperDashToggle", {
    Title = "Super Dash", 
    Description = "Make Your Dash Longer Lol",
    Default = false
})
...
v178:OnChanged(function(state)
    ...
    if player and player.Character then
        ...
        if humanoid then
            if state then
                player.Character:SetAttribute("DashLength", 300) -- Enable Super Dash
            else
                player.Character:SetAttribute("DashLength", 1)   -- Reset to default
            end
        end
    end
end)
...
-- Inf Stamina
local InfiniteStaminaToggle = v16.Hunt:AddToggle("InfiniteStaminaToggle", {
...
spawn(function()
    while wait(0.3) do
        if _G.InfiniteEnergy and player.Character and player.Character:FindFirstChild("Energy") then
            storedEnergy = player.Character.Energy.Value
            InfiniteStamina()
        end
    end
end)
...
-- Dash No CD
local DashNoCooldownToggle = v16.Hunt:AddToggle("DashNoCooldownToggle", {
    Title = "Dash No Cooldown",
    Description = "Unlimted Dash"
})

_G.DashNoCd = false -- Set to false to disable

function DodgeNoCoolDown()
    local player = game.Players.LocalPlayer
    if not player or not player.Character then return end

    local dodgeScript = player.Character:FindFirstChild("Dodge")
    if not dodgeScript then return end

    for _, v in next, getgc(true) do
        if type(v) == "function" and islclosure(v) then
            local env = getfenv(v)
            if env and env.script == dodgeScript then
                for index, value in next, getupvalues(v) do
                    if type(value) == "number" and value == 0.4 then
                        setupvalue(v, index, 0)
                    end
                end
            end
        end
...
-- Loop to ensure cooldown stays at 0
spawn(function()
    while _G.DashNoCd do
        DodgeNoCoolDown()
        wait(0.3) -- More frequent to ensure cooldown stays removed
    end
end)
...
-- Spectate Player
_G.Spectating = false
_G.SelectPly = nil -- No default player
...
function SpectatePlayer()
    ...
    -- Ensure a valid target player is selected
    if not _G.SelectPly or _G.SelectPly == "" then
    ...
    if not targetPlayer or not targetPlayer.Character then 
        warn("Invalid player selected or player not in-game!")
        StopSpectating()
        return
    end
    ...
    -- Stop spectating if the target dies or leaves
    local connection
    connection = targetPlayer.CharacterRemoving:Connect(function()
    ...
    print("Now spectating:", _G.SelectPly)
end
...
-- Toggle Spectate
ToggleSpectate:OnChanged(function(state)
    if state then
        SpectatePlayer()
    else
        StopSpectating()
...
Title = "Lucky Hub Goddess",

Content = "Lucky Hub Loaded",
